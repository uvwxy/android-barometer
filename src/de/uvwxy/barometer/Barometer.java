...
public class Barometer {
    ...
    double[][] pressureValueSteps = new double[][] {
    ...
    String[] pressureNames = new String[] { "%d mb", "%.3f bar", "%d Pa", "%d hPa", "%.3f at", "%.3f atm", "%d Torr",
            "%.3f psi", };
    String[] pressureValueDisplayFormat = new String[] { "%.0f", "%.3f", "%.0f", "%.0f", "%.3f", "%.3f", "%.0f",
            "%.3f", };
    ...
    private float millibarToDegrees(float millibars, int typePressure) {

        // calc how many millibars per step

        float valueStepSize = (float) (pressureValueSteps[typePressure - 1][1] - pressureValueSteps[typePressure - 1][0]);
        Unit typePressureUnit = FragmentSettings.getPressureUnit(typePressure);

        // refresh: degreesPerMilliBar
        typePressureUnit.setValue(valueStepSize);
        float millsPerStep = (float) typePressureUnit.to(Unit.from(Unit.MILLI_BAR)).getValue();
        float degreesPerMilliBar = degreesPerStep / millsPerStep;

        // refresh: firstStepInMillis
        typePressureUnit.setValue(pressureValueSteps[typePressure - 1][0]);
        float firstStepInMillis = (float) typePressureUnit.to(Unit.from(Unit.MILLI_BAR)).getValue();

        // refresh: deg
        float deg = firstStepDegrees + (millibars - firstStepInMillis) * degreesPerMilliBar;
    ...
    protected Bitmap drawUnitsWithMemory(float millisCurrent, float millisMemory, int typePressure, int typeLength) {
        ...
        matrix.setRotate(millibarToDegrees(millisCurrent, typePressure), canvas.getWidth() / 2, canvas.getHeight() / 2);
        canvas.drawBitmap(handle, matrix, paint);

        matrix.setRotate(millibarToDegrees(millisMemory, typePressure), canvas.getWidth() / 2, canvas.getHeight() / 2);
    ...
    protected Bitmap drawUnits(int type) {
        ...
        if (type > pressureValueSteps.length) {
            return bm;
        }
        ...
        for (int i = 0; i < pressureValueSteps[type - 1].length; i++) {
            canvas.drawText(String.format(pressureValueDisplayFormat[type - 1], pressureValueSteps[type - 1][i]),
